...
public final class Cakes implements EventLoop {

    private final Client client = RBApi.getClient();

    private final Random random = new Random();

    private final WorldPoint cakeTile = new WorldPoint(2669, 3310, 0);

    private final WorldPoint runTile = new WorldPoint(2679, 3313, 0);

    private final int foodId;

    private static final int[] droppableItems = {ItemID.CHOCOLATE_CAKE, ItemID.CAKE, ItemID.BREAD};
    ...
    Cakes(int foodId, Plugin caller)
    {
        this.foodId = foodId;
    ...
    {
        if (LocationInformation.isOnTile(cakeTile)) lock = false;
        if (LocationInformation.isOnTile(runTile)) Movement.move(cakeTile);
        if (lock) return true;
        ...
        Optional<TileObject> cakeStall = TileObjects.search().withId(ObjectID.BAKERS_STALL_11730).atLocation(2667, 3310, 0).first();

        cakeStall.ifPresent(obj ->  {
